...
- Database Tool，支持 SQL 数据库和 MongoDB
- 基于环境变量的配置加载
...
- 配置数据库连接后执行数据库查询
- 面向 SQLite、MySQL、PostgreSQL、MongoDB 等主流数据库
...
.venv/bin/python -m pip install -e '.[dev,mongodb,mysql,postgres]'
```
...
- SQLite：通过 SQLAlchemy
- MySQL：通过 SQLAlchemy + `pymysql`
- PostgreSQL：通过 SQLAlchemy + `psycopg`
- MongoDB：通过 `pymongo`
